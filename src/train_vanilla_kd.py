"""
Vanilla Knowledge Distillation baseline for KLUE-NLI.

This script trains a student Transformer model using soft labels
from a teacher Transformer model.
"""
...
def preprocess_dataset(dataset, tokenizer, max_length=128):
    """
    Tokenize and format KLUE-NLI dataset.
    """
...
def kd_loss_fn(student_logits, teacher_logits, labels, temperature=2.0, alpha=0.5):
    """
    Compute vanilla knowledge distillation loss.

    The total loss contains:
    1. Cross-entropy loss with ground-truth labels.
    2. KL-divergence loss between teacher and student soft predictions.
    """
...
def main():
    ...
    # Small subset for quick baseline experiment.
    ...
    print("Training student model with Vanilla KD...")
    ...
    print("Evaluating Vanilla KD student model...")
    results = evaluate_model(student_model, eval_loader, device)

    print(results)

    output_path = os.path.join(result_dir, "vanilla_kd_klue_nli_results.txt")
